import * as React from "react";
import { useInstanceSuffix } from "../../useInstanceSuffix";

export const GeyangGu = (props: React.SVGProps<SVGSVGElement>) => {
  const sfx = useInstanceSuffix();
  return (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    overflow="visible"
    viewBox="0 0 758 700"
    {...props}
  >
    <defs>
      <linearGradient
        id={`kicon-geyanggu-b${sfx}`}
        x1={378}
        x2={378}
        y1={505}
        y2={727}
        gradientUnits="userSpaceOnUse"
        spreadMethod="reflect"
      >
        <stop offset={0} stopColor="#292C8D" />
        <stop offset={0.09} stopColor="#292C8D" />
        <stop offset={1} stopColor="#71CAEF" />
      </linearGradient>
      <clipPath id={`kicon-geyanggu-a${sfx}`}>
        <path d="M-1 28h758v700H-1z" />
      </clipPath>
    </defs>
    <g fillRule="evenodd" clipPath={`url(#kicon-geyanggu-a${sfx})`} transform="translate(1 -28)">
      <path
        fill="#DCDDDD"
        d="M564 594.526C564 620.223 480.739 641 378.026 641S192 620.171 192 594.526C192 568.83 275.261 548 378.026 548 480.739 548 564 568.83 564 594.526Z"
      />
      <path
        fill="#00A652"
        d="M377.133 145.408C420.236 304.307 433.887 458.703 403.961 601 412.729 423.508 319.382 241.093 156 115.084c23.1-43.889 78.751-98.985 141.91-83.378 50.82 9.427 67.463 70.337 79.223 113.702Z"
      />
      <path
        fill="#00A652"
        d="M281.555 278.958C348.564 369.239 380.864 473.773 385 576 351.077 445.74 227.634 325.802 71 276.705c17.119-49.516 40.153-93.372 69.574-136.705 76.851 57.637 100.566 84.517 140.981 138.958Z"
      />
      <path
        fill="#00A652"
        d="M281.633 437.264C316.807 469.829 343.581 509.252 365 557c-27.666-35.811-52.498-65.234-98.381-71.674-61.685-8.743-112.976 20.785-168.204 4.084-80.165-24.24-42.26-184.918-36.014-183.399 86.15 21.361 158.544 75.024 219.232 131.253Z"
      />
      <path
        fill="#AFD144"
        d="M691 343.25C564.492 364.45 507.595 396.04 429 533c4.518-27.654 4.991-64.282 7.408-89.68 16.233-40.143 55.321-111.667 93.252-144.936 33.308-29.281 80.959-56.253 135.124-65.384 19.807 60.661 21.278 84.17 26.216 110.25Z"
      />
      <path
        fill="#AFD144"
        d="M676.376 464.974c-33.001 35.677-87.708 15.2-121.285 16.663C495.154 484.249 465.813 519.927 428 566c30.857-55.737 69.246-121.556 159.517-171.912 24.006-13.373 74.686-28.94 103.974-28.052 6.799 32.805 4.707 77.52-15.115 98.938ZM600.925 228.328c-126.7 51.165-165.608 187.745-163.665 177.081.683-6.934 2.521-56.838-1.26-117.143 28.301-81.79 56.445-149.03 141.507-154.178 38.855-2.363 73.09 43.181 80.493 79.112.053-.053-40.746 8.51-57.075 15.128Z"
      />
      <path
        fill={`url(#kicon-geyanggu-b${sfx})`}
        d="M547.644 511.219c-7.921 4.181-18.412 10.347-26.071 16.984 29.428 4.286 55.918 9.825 78.475 16.41 50.043 14.581 80.625 34.073 80.625 55.552 0 45.153-135.652 81.787-302.988 81.787-167.335 0-303.04-36.634-303.04-81.787 0-35.693 84.77-66.056 202.954-77.24 1.731-.157 3.514-.261 5.193-.418-7.029-7.63-16.891-13.483-25.284-17.507C107.85 520.208 0 562.799 0 613.021 0 675.942 169.224 727 378 727s378-51.006 378-113.979c0-44.525-84.822-83.093-208.356-101.802Z"
      />
    </g>
  </svg>
  );
};

export default GeyangGu;
